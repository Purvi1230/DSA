...
vector<lli> karatsuba (vector <lli> &A, vector <lli> &B){
    ...
    lli k = n>>1; //>> right shift operator
    vector<lli> Al(A.begin(), A.begin()+k);
    vector<lli> Ar(A.begin()+k, A.end());
    vector<lli> Bl(A.begin(), A.begin()+k);
    vector<lli> Br(A.begin()+k, A.end());

    //Recursive calls;
    vector<lli> AlBl = karatsuba(Al, Bl);
    vector<lli> ArBr = karatsuba(Ar, Br);

    for(lli i = 0; i<(lli)Al.size(); i++)
        Al[i]+=Ar[i];
    for(lli i = 0; i<(lli)Bl.size(); i++)
        Bl[i]+=Br[i];
    ...
    for(lli i = 0; i<(lli)Zm.size(); i++)
        Zm[i]-=AlBl[i];
    for(lli i = 0; i<(lli)Zm.size(); i++)
        Zm[i]-=ArBr[i];
    for(lli i = 0; i<(lli)Zm.size(); i++)
        ans[i+k] += Zm[i];
    for(lli i = 0; i<(lli)AlBl.size(); i++)
        ans[i] += AlBl[i];
    for(lli i = 0; i<(lli)ArBr.size(); i++)
        ans[i+n] += ArBr[i];
    return ans;
}
